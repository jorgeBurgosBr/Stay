...
if ($bd->conectar()) {
   ...
   if (isset($_GET['id'])) {
      // Obtén el valor del parámetro "id" y asígnalo a una variable
      $id = $_GET['id'];
      ...
      $sql = "SELECT p.*, pp.*, np.*
              FROM paciente p
              JOIN perfil_paciente pp ON pp.id_paciente = p.id_paciente
              JOIN notas_paciente np ON np.id_paciente = p.id_paciente
              WHERE p.id_paciente = ?";
