...
function setEventListeners(){
    ...
    call.addEventListener('click', function(){
        if(call.textContent == 'call'){
            ...
            camera_section[0].style.background = `#F5F5F5 url(./img/loading2.gif) center / 30px 30px no-repeat`;
            camera_section[1].style.background = `#F5F5F5`;
            ponerFoto(camera_section[1]);
        } else{
            call.style.color = 'green';
            call.textContent = 'call';
            camera_section[0].style.background = `url(./svg/camera_section.svg) center / cover no-repeat`;
            camera_section[1].style.background = `url(./svg/camera_section.svg) center / cover no-repeat`;
...
function ponerFoto(div){
    fetch('./php/procesar_videollamada_paciente.php')
    ...
    .then(data => {
        if (data.success) {
          ...
          console.log('No tienes fotos para este paciente');
        }
      })
      .catch(error => {
